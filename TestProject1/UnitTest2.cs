...
namespace TestProject1
{
    public class ClaimApprovalTest
    {
        ...
        public ClaimApprovalTest()
        {
            ...
            {
                Id = Guid.NewGuid().ToString(),  //generate a unique user id using guid
                Email = "nikita@cmcs.com", //set user's email
                UserName = "nikita@cmcs.com" //set user's username
            };
        ...
        {
            //arrange: seed a claim with "pending" status
            var claim = new Claims
            {
                LecturerID = Guid.NewGuid().ToString(),  //use a unique lecturer id
                FirstName = "Anna", //set first name
                LastName = "Perumal", //set last name
                Email = "anna@dlssa.com", //set email
                ClaimsPeriodStart = DateTime.Now.AddDays(-7), //set claim start date 7 days ago
                ClaimsPeriodEnd = DateTime.Now, //set claim end date as today
                ...
                DateSubmitted = DateTime.Now //set the current date for submission
            };

            _context.Claims.Add(claim); //add claim to the context
            await _context.SaveChangesAsync(); //save changes asynchronously

            //act: change the status to "processed"
            await _controller.UpdateClaimStatus(claim.ID, "Processed");

            //assert: verify that the claim's status is now "processed" in the database
            var updatedClaim = await _context.Claims.FindAsync(claim.ID); //fetch the claim from the db
            Assert.NotNull(updatedClaim); //ensure the claim is found
            Assert.Equal("Processed", updatedClaim.ClaimStatus); //check that status is updated to "processed"
        }
    }
}
